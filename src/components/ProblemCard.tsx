...
export const ProblemCard = ({ problem, onUpdateStatus, onDelete, onEdit, currentUserId, onAssignToUser, members }: ProblemCardProps) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 mb-3 relative transition-all hover:shadow-md" style={{ zIndex: isStatusOpen ? 50 : 'auto' }}>
      ...
      <div className="flex justify-between items-start mb-2">
        <div className="flex-1 pr-12">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            <span className={`text-[10px] px-2 py-0.5 rounded-md font-medium ${
              problem.difficulty === 'Hard' ? 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400' :
              problem.difficulty === 'Medium' ? 'bg-orange-50 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400' :
              'bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400'
            }`}>
              {problem.difficulty}
            </span>
            {problem.tags?.map((tag: string) => (
              <span key={tag} className="text-[10px] px-1.5 py-0.5 bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 rounded">
                {tag}
              </span>
            ))}
          </div>
          <h3 className="font-semibold text-gray-900 dark:text-gray-100 leading-tight">{problem.title}</h3>
        </div>
        ...
        <div className="relative z-20" ref={statusMenuRef}>
          ...
          {isStatusOpen && canEdit && (
            <div className="absolute right-0 mt-1 w-32 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 overflow-hidden z-[100] animate-in slide-in-from-top-2 duration-200">
        ...
        <div className="flex -space-x-2">
           {problem.assignees?.map((userId: string) => {
             const member = members?.find(m => m.uid === userId);
               ...
               return (
                 <div key={userId} title={titleText} className={`w-6 h-6 rounded-full border-2 border-white dark:border-slate-800 flex items-center justify-center text-[10px] text-white ${avatarClass} relative overflow-hidden`}>
                   {photoURL ? (
           ...
           {canEdit && members && members.length > 0 && (
             <div className="relative" ref={assigneeMenuRef}>
               <button 
                 ...
                 className="w-6 h-6 rounded-full border-2 border-dashed border-gray-300 dark:border-slate-600 flex items-center justify-center text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-slate-700 hover:bg-gray-100 dark:hover:bg-slate-600 text-[10px] relative z-10 cursor-pointer transition-colors"
                 title="Assign to teammate"
               >
                 <UserPlus size={12} />
               </button>
               {isAssigneeOpen && (
                 <div className="absolute left-0 mt-1 w-48 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 overflow-hidden z-[100] animate-in slide-in-from-top-2 duration-200">
                   {members
                     ...
                     .map((member) => (
                       <button
                         ...
                         className="w-full px-3 py-2 text-left text-xs font-medium transition-colors hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-700 dark:text-gray-300 flex items-center gap-2"
                       >
                   ...
                   {members.filter(m => !problem.assignees?.includes(m.uid)).length === 0 && (
                     <div className="px-3 py-2 text-xs text-gray-400 dark:text-gray-500 text-center">All teammates assigned</div>
                   )}
                 </div>
               )}
             </div>
          ...
          {problem.url && (
            <a href={problem.url} target="_blank" rel="noopener noreferrer" className="text-gray-400 dark:text-gray-500 hover:text-blue-500">
              <ExternalLink size={16} />
            </a>
          )}
          {canEdit && onEdit && (
            <button onClick={() => onEdit(problem)} className="text-gray-400 dark:text-gray-500 hover:text-blue-500">
              <Edit size={16} />
            </button>
          )}
          {canEdit && (
            <button onClick={() => onDelete(problem.id)} className="text-gray-400 dark:text-gray-500 hover:text-red-500">
              <Trash2 size={16} />
            </button>
          )}
        </div>
